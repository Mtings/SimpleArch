...
import com.song.sakura.helper.PickerLayoutManager;
...
public final class TimeDialog {
    ...
    public static final class Builder
        ...
        public Builder(Context context) {
            ...
            mHourAdapter = new PickerAdapter();
            mMinuteAdapter = new PickerAdapter();
            mSecondAdapter = new PickerAdapter();

            // 生产小时
            ArrayList<String> hourData = new ArrayList<>(24);
            for (int i = 0; i <= 23; i++) {
                hourData.add((i < 10 ? "0" : "") + i + " " + "时");
            }

            // 生产分钟
            ArrayList<String> minuteData = new ArrayList<>(60);
            for (int i = 0; i <= 59; i++) {
                minuteData.add((i < 10 ? "0" : "") + i + " " + "分");
            }

            // 生产秒钟
            ArrayList<String> secondData = new ArrayList<>(60);
            for (int i = 0; i <= 59; i++) {
                secondData.add((i < 10 ? "0" : "") + i + " " + "秒");
            }
    ...
    private static final class PickerAdapter extends BaseQuickAdapter<String, BaseViewHolder> {
        ...
        protected void convert(BaseViewHolder helper, String item) {
            ((TextView) helper.findViewById(R.id.tv_picker_name)).setText(item);
        }
    }
